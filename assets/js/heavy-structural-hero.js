/* ════════════════════════════════════════════════════════════════════
   HEAVY STRUCTURAL HERO — vanilla JS, no animation library.
   Owns: entrance reveal (single .hst-loaded class flip, CSS does the
   animating), a drifting spark particle field weighted toward the
   structural-steel side of the photo, and a light mouse parallax across
   background depth layers only (text never moves). Mirrors
   ferro-hero.js / aluminium-hero.js / steel-hero.js.
   ════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  const hero = document.querySelector('.hst-hero');
  if (!hero) return;

  const bg = hero.querySelector('.hst-bg');
  const frame = hero.querySelector('.hst-visual-frame');
  const curve = hero.querySelector('.hst-curve');
  const particleHost = hero.querySelector('#hstParticles');

  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const canHover = window.matchMedia('(hover: hover)').matches;

  requestAnimationFrame(() => {
    hero.classList.add('hst-loaded');
  });

  // ── drifting spark particles: plain spans positioned once with
  ...
  // hero (structural steel / beam stack) rather than the full width ──
