/* ════════════════════════════════════════════════════════════════════
   RAW & INTERMEDIATE MATERIALS HERO — vanilla JS, no animation library.
   Owns: entrance reveal (single .rm-loaded class flip, CSS does the
   ...
   gradient, the reference artwork and the text column. Particles and
   parallax are desktop/no-reduced-motion only. Mirrors steel-strip-
   hero.js.
   ════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  const hero = document.querySelector('.rm-hero');
  if (!hero) return;

  const bg = hero.querySelector('.rm-bg');
  const frame = hero.querySelector('.rm-visual-frame');
  const content = hero.querySelector('.rm-content');
  const particleHost = hero.querySelector('#rmParticles');

  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const canHover = window.matchMedia('(hover: hover)').matches;

  requestAnimationFrame(() => {
    hero.classList.add('rm-loaded');
  });
  ...
  // side (5%-55%) rather than the artwork — the artwork already has its
  ...
  // artwork (opposite direction, slightly stronger for depth), text
  // column (1-2px). The whole composition stays visually unified —
  // nothing separates from the artwork. ──
