/* ════════════════════════════════════════════════════════════════════
   ...
   Compromise" section. Isolated component (see quality-carousel.css).
   Guards on its own root element (#qccCarousel) so it's a no-op on any
   page that doesn't include the markup. Same coverflow positioning
   model as process-carousel.js (circular signed distance from the
   active index, placed via --tx/--sc/--op/--z custom properties).

   Each slide here IS a link (.pillar-card → quality.html#qc-N). A plain
   click-to-recenter on every slide would hijack navigation on the
   active card too, so only off-center slides intercept the click to
   recenter; the centered card's click is left alone and navigates
   normally.
   ════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  const root = document.getElementById('qccCarousel');
  if (!root) return;

  const trackWrap = root.querySelector('.qcc-track-wrap');
  const track = root.querySelector('.qcc-track');
  const slides = Array.from(track.querySelectorAll('.qcc-slide'));
  const total = slides.length;
  if (!total) return;

  const prevBtn = root.querySelector('.qcc-prev');
  const nextBtn = root.querySelector('.qcc-next');
  const dotsWrap = root.querySelector('.qcc-dots');
  const status = root.querySelector('.qcc-sr-status');
  ...
  const dots = slides.map((_, i) => {
    ...
    btn.type = 'button';
    btn.className = 'qcc-dot';
    btn.setAttribute('role', 'tab');
    btn.setAttribute('aria-label', 'Go to checkpoint ' + (i + 1) + ' of ' + total);
  ...
  function render() {
    slides.forEach((slide, i) => {
      ...
      const card = slide.querySelector('.qcc-card');
  ...
  // ── cursor spotlight (mirrors process-carousel.js, scoped to
  // .qcc-card only — rAF-coalesced so a fast mouse sweep never forces
